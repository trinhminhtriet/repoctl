...
- Background to `repoctl` and core design decisions used to develop `repoctl`
- Comparisons with alternatives
- Roadmap

## Background

`repoctl` came about because I needed a CLI tool to manage multiple repositories. So, the premise is, you have a bunch of repositories and want the following:
...
Now, there's plenty of CLI tools for running cloning multiple repositories, running commands over them, see [similar software](#similar-software), and while I've taken a lot of inspiration from them, there's some core design decision that led me to create `repoctl`, instead of forking or contributing to an existing solution.

## Design

### Config

A lot of the alternatives to `repoctl` treat the config file (either using a custom format or JSON) as a state file that is interacted with via their executable.
...
That's why in `repoctl` you need to edit the config file to add or delete a repository. The exception is when you're setting up `repoctl` for the first time, then you want it to scan for existing repositories. As a bonus, it also updates your `.gitignore` file with the updated list of repositories.
...
Note, you can still use `make` or regular script files, just call them from the `repoctl.yaml` config.
...
When we run commands, we need a way to target specific repositories. To make it as flexible as possible, there are three ways to do it in `repoctl`:

1. **Tag filtering**: target repositories which have a tag, for instance, add a tag `python` to all `python` repositories, then it's as simple as `repoctl run status -t python`
2. **Directory filtering**: target repositories by which directory they belong to, `repoctl run status -d frontend`, will target all repositories that are in the `frontend` directory
3. **Project name filtering**: target repositories by their name, `repoctl run status -p rmrfrs`, will target the project `rmrfrs`

### General UX

These various features make using `repoctl` feel more effortless:

- Automatically updating .gitignore when updating the config file
- Rich auto-completion
- Edit the `repoctl` config file via the `repoctl edit` command, which opens up the config file in your preferred editor
