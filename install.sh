...
url=$(
    fetch https://api.github.com/repos/trinhminhtriet/repoctl/releases/latest |
...
echo "Downloading repoctl..."

temp_dir=$(mktemp -dt repoctl.XXXXXX)
trap 'rm -rf "$temp_dir"' EXIT INT TERM
cd "$temp_dir"

if ! fetch repoctl.tar.gz "$url"; then
...
tar xzf repoctl.tar.gz

if test -w "$bindir"; then
    mv repoctl "$bindir/"
else
    sudo mv repoctl "$bindir/"
fi

$bindir/repoctl version
