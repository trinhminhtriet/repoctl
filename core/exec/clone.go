...
func UpdateGitignoreIfExists(config *dao.Config) error {
	// Only add projects to gitignore if a .gitignore file exists in the repoctl.yaml directory
	...
	if _, err := os.Stat(gitignoreFilename); err == nil {
		...
		for _, project := range config.ProjectList {
			...
			// Project must be below repoctl config file to be added to gitignore
