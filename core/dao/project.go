...
// We only return error if the flags provided do not exist in the repoctl config.
 ...
 * Example:
 * Input:
 *   - /trinhminhtriet/tools/project-a
 *   - /trinhminhtriet/tools/project-b
 *   - /trinhminhtriet/tools/node/project-c
 *   - /backend/project-d
 * Output:
 *   - /frontend
 *   - /trinhminhtriet/tools
 *   - /trinhminhtriet/tools/node
 *   - /backend
 */
func (c Config) GetProjectPaths() []string {
	dirs := []string{}
	for _, project := range c.ProjectList {
		// Ignore projects outside of repoctl.yaml directory
...
func UpdateProjectsToGitignore(projectNames []string, gitignoreFilename string) error {
	...
	const repoCtlComment = "# repoctl #"
	var insideComment = false
	var beginElement *list.Element
	var endElement *list.Element
	var next *list.Element

	// Remove all projects inside # repoctl #
	for e := l.Front(); e != nil; e = next {
		next = e.Next()

		if e.Value == repoCtlComment && !insideComment {
			insideComment = true
			beginElement = e
			continue
		}

		if e.Value == repoCtlComment {
			endElement = e
			break
		}
	...
	// If missing start # repoctl #
	if beginElement == nil {
		l.PushBack(repoCtlComment)
		beginElement = l.Back()
	}

	// If missing ending # repoctl #
	if endElement == nil {
		l.PushBack(repoCtlComment)
	}

	// Insert projects within # repoctl # section
